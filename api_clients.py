...
class ExchangeApiClient:
    ...
    def fetch_rates(self):
        if not self.api_key:
            logger.error("Ключ API от ExchangeApi не предоставлен!")
            self.success = False
            return
        ...
        try:
            ...
            if data.get("result") == "success":
                ...
                logger.warning(
                    f"API ExchangeApi вернул ошибку: {data.get('error_type')}"
                )
                self.success = False

        except requests.exceptions.RequestException as e:
            logger.exception("Ошибка сети при запросе к ExchangeApi")
            self.success = False
...
class CBRFClient:
    ...
    def fetch_rates(self):
        if not self.full_url:
            logger.error("URL от API ЦБ РФ не предоставлен!")
            self.success = False
            return
        ...
        try:
            ...
            logger.exception("Ошибка сети при запросе к ЦБ РФ")
            self.success = False
        except ET.ParseError as e:
            logger.exception("Ошибка парсинга XML ответа от ЦБ РФ")
            self.success = False